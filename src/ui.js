...
export function displayTab() {
  ...
  todos.forEach((todo) =>
    todosContainer.appendChild(createTodoEntry(todo, todo.id))
  );
}

function createTodoEntry(todo) {
  const todoEntry = todoTemplate.content
    .cloneNode(true)
    .querySelector(".todo-entry");
...
function clickOutsideCallback(e) {
  ...
  if (
    e.target !== addTodoForm &&
    !addTodoForm.contains(e.target) &&
    e.target !== addTodoButton &&
    !addTodoButton.contains(e.target)
  ) {
    hideForm();
